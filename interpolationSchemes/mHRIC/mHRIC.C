...
) const
{
    ...
    scalar upwindWeight = pos(sign(faceFlux));	//-RM: pos(sign()) as a replacement to pos0.
...
) const
{
    ...
    scalarField& pLim = lim.dimensionedInternalField();
...
) const
{
    ...
    scalarField& w = weightingFactors.dimensionedInternalField();
